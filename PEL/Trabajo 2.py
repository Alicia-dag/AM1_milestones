...
def Propiedades_químicas(Tipo_mezcla, Tipo_mezcla_valores):
    if Tipo_mezcla == Tipo_mezcla_valores[0]: # Mezcla rica
        ...
        Q_comb = - delta_f_prod * (1 + 1 / OF_PC_st) # Calor de reacción [J/mol]
        Q_comb_kg = Q_comb / M_H2O
        Q_comb_kg = (- (a * delta_f_H2O) * (1 + 1 / OF_PC_st)) / M_H2O
    ...
    return OF, a, b, c, Cp_prod, Mp_prod, Cp_prod_kg, delta_f_prod, Q_comb, Q_comb_kg, Rg, gamma, Gamma_gamma, Tc
...
OF_PC, a_PC, b_PC, c_PC, Cp_prod_PC, Mp_prod_PC, Cp_prod_PC_kg, delta_f_prod_PC, Q_comb_PC, Q_comb_PC_kg, Rg_PC, gamma_PC, Gamma_gamma_PC, Tc_PC = Propiedades_químicas(Tipo_mezcla_PC, Tipo_mezcla_PC_valores)
...
def Ec_Energia(OF, OF_st, Cp_prod_kg, Tc, T_ref, Q_comb_kg, eta_q, Tipo_mezcla, Tipo_mezcla_valores):
    
    if Tipo_mezcla == Tipo_mezcla_valores[0]: # Mezcla rica
        residual =  (1 + OF) / (OF_st) * Cp_prod_kg * (Tc - T_ref) - Q_comb_kg * eta_q
        eq = Eq(residual, 0)
...
if Tipo_mezcla_PC == Tipo_mezcla_PC_valores[0]: # Mezcla rica
    soluciones = Ec_Energia(OF_PC, OF_PC_st, Cp_prod_PC_kg, Tc_PC, T_ref, Q_comb_PC_kg, eta_q, Tipo_mezcla_PC, Tipo_mezcla_PC_valores)
...
# tau_t = (m_f * tau_B1_f + m_ox * tau_B2_ox) / (eta_mec * m_t)
...
#     print("Seleccione el tipo de mezcla en la cámara de combustión:")
#     print("1. Rica")
#     print("2. Pobre")
...
# OF_CC, a_CC, b_CC, c_CC, Cp_prod_CC, Mp_prod_CC, Cp_prod_CC_kg, delta_f_prod_CC, Q_comb_CC, Rg_CC, gamma_CC, Gamma_gamma_CC, Tc_CC = Propiedades_químicas(Tipo_mezcla_CC, Tipo_mezcla_CC_valores)
...
# p_CC_s = p_adapt
# p_PC_s = pc_PC / (1 + (gamma_PC - 1) / 2) ** (gamma_PC / (gamma_PC - 1))
